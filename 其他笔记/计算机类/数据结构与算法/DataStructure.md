...
下方是算法分析的两个定理：
<details>
<summary><strong>求和定理</strong></summary>

定理 1.1 求和定理。 假定 $T_1(n)$、$T_2(n)$ 是程序段 $\text{P}_1$、$\text{P}_2$ 的运行时间，并且 $T_1(n)$ 是 $O(f(n))$ 的，而 $T_2(n)$ 是 $O(g(n))$ 的。那么，先运行 $\text{P}_1$、再运行 $\text{P}_2$ 的总的运行时间是 $T_1(n) + T_2(n) = O(\text{MAX}(f(n), g(n)))$。
</details>

<details>
<summary><strong>求积定理</strong></summary>

定理 1.2 求积定理。 如果 $T_1(n)$ 和 $T_2(n)$ 分别是 $O(f(n))$ 和 $O(g(n))$ 的，那么 $T_1(n) \times T_2(n)$ 是 $O(f(n) \times g(n))$。证明： 根据已知条件，可得在 $n \ge n_1$ 时，$T_1(n) \le c_1 f(n)$ 成立。在 $n \ge n_2$ 时，$T_2(n) \le c_2 g(n)$ 成立。其中 $c_1, n_1$ 及 $c_2, n_2$ 都是常数。所以，在 $n \ge \text{MAX}(n_1, n_2)$ 时，$T_1(n) \times T_2(n) \le c_1 c_2 f(n) g(n)$，所以 $T_1(n) \times T_2(n)$ 是 $O(f(n) \times g(n))$ 的。
</details>
